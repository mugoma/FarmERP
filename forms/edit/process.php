...
$name_err = $product_err = $requirements_err = $worker_err =$notes_err= $process_id_err="";
...
if (isset($_GET['process_id']) && test_int($_GET['process_id'])){
    ...
    if($process_fields=pg_fetch_assoc($process_details)){
        ...
        $worker=array_column(pg_fetch_all(pg_query($link, "SELECT  erp_farm_process_worker.worker_id FROM erp_farm_process_worker WHERE farm_process_id=$process_id")) ,'worker_id');
        $product=array_column(pg_fetch_all(pg_query($link, "SELECT product_id FROM erp_farm_process_product WHERE farm_process_id=$process_id")), 'product_id');
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($product_err) && empty($requirements_err) && empty($worker_err) && empty($notes_err)){
        ...
        if((pg_prepare($link,'stmt_insert', $sql)) 
            ...
            $requirement_dict.="}";
            $exec_1=pg_execute($link, 'stmt_insert_4', array($process_id+0));
            $exec_2=pg_execute($link, 'stmt_insert_5', array($process_id+0));
            ...
            if($result && $exec_1 && $exec_2 && empty($product) && empty($worker) && pg_query($link, 'COMMIT;')){
                $name= $requirements = $product = $worker = $notes=$process_id="";
                $process_query=pg_query($link, $process_sql);
                $process_list=pg_fetch_all($process_query);
                ...
                if(!empty($product)){
                        ...
                        $exec_4=pg_execute($link, 'stmt_insert_2', array_reverse($product)) ;
                }
                if(!empty($worker)){
                        array_push($worker, $process_id);
                        $exec_5=pg_execute($link, 'stmt_insert_3', array_reverse($worker))  ;

                    }

            //}   else{
                if ($result && $exec_1 && $exec_2 && $exec_4 && $exec_5){
...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Edit Existing Farm Process<?=" | Forms | ".SOFTWARE_NAME ?></title>
            ...
            <div class="wrapper container">
                ...
                <form  id='form' method="<?= (!empty($process_id)) ? 'post' : 'get'; ?>">
                    <div class="form-group ">
                        ...
                        <select class="form-control select_multiple <?= (!empty($process_err)) ? 'is-invalid' : ''; ?>" id="process" name="process_id" onchange="send_get_request('process')">
                    ...
                    <?php if(!empty($process_id) && !empty($product_list)){ ?>
                    <div class="form-group ">
