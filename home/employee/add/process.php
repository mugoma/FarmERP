...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($product_err) && empty($requirements_err) && empty($worker_err) && empty($notes_err)){
        ...
        $sql = "INSERT INTO erp_farm_process (name, requirements, notes)VALUES ($1, $2, $3) RETURNING id";
        $sql_2 = $sql_3 = '';
        if (!empty($product)){
            $rows="";
        ...
        if(pg_prepare($link,'stmt_insert', $sql) 
            ...
            $param_sql = array($name, $requirement_dict, $notes);
