...
$name= $sold = $purchase = $unit = $notes=$grows=$consumable=$product_id=$grows_prev=$prows_now="";
$name_err = $purchase_err = $sold_err = $unit_err =$notes_err=$grows_err=$consumable_err=$consumable_grow_err=$product_id_err="";
...
if (isset($_GET['product_id']) && test_int($_GET['product_id'])){
    ...
    if(empty($product_fields)){
        ...
        $prev_name=$product_fields[0]['name'];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(test_input($_POST["name"]))){
        ...
        $current_name = test_input($_POST["name"]);
...
;
    ...
    if(empty($name_err) 
        ...
        pg_query($link, 'BEGIN;');
        $sql = "UPDATE  erp_product SET name=$1, sale=$2, purchase=$3, unit_of_measure_id=$4, notes=$5, grows=$6, consumable=$7 WHERE id= $8 ;";
        ...
        if(pg_prepare($link,'stmt_insert', $sql)){
            ...
            $array=array($param_name, $param_sold, $param_purchase, $param_unit, $param_notes, $param_grows,$param_consumable, $param_product_id);
...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Edit Existing Product | Yengas FarmERP</title>
    ...
    <body>
        ...
        <main>
            <div class="wrapper container">
                ...
                <form  id='form' method="<?php echo (!empty($product_id)) ? 'post' : 'get'; ?>">
                    <div class="form-group ">
                        <label for='process'>Product:</label>
                        <select class="form-control select_multiple <?php echo (!empty($name_err)) ? 'is-invalid' : ''; ?>" id="product" name="product_id" onchange="send_get_request('product')">
                    ...
                    <div class='form-row'>
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input <?php echo (!empty($grows_err)) ? 'is-invalid' : ''; ?>" type="checkbox" value="true" id="grows" name="grows" <?php echo ($grows=='true') ? 'checked' : ''; ?> disabled>
